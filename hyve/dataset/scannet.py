...
class ScanNet(pl.LightningDataModule):
    """
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        return DataLoader(DynamicDataset(self.train_files_list, knn_instead_of_mesh=self.knn_instead_of_mesh, subsample=self.subsample, noise_scale_normal=self.noise_scale_normal, noise_scale_uniform=self.noise_scale_uniform), batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers, pin_memory=True, drop_last=True)

    def val_dataloader(self) -> TRAIN_DATALOADERS:
        return DataLoader(DynamicDataset(self.test_files_list, knn_instead_of_mesh=self.knn_instead_of_mesh, noise_scale_normal=self.noise_scale_normal, noise_scale_uniform=self.noise_scale_uniform, subsample=self.subsample_val), batch_size=self.batch_size_test, num_workers=self.num_workers, pin_memory=True)

    def test_dataloader(self) -> TRAIN_DATALOADERS:
        if self.gt_mesh_files_list is None:
            return DataLoader(DynamicDataset(self.test_files_list, knn_instead_of_mesh=self.knn_instead_of_mesh, noise_scale_normal=self.noise_scale_normal_test, noise_scale_uniform=self.noise_scale_uniform_test, subsample=self.subsample_test), batch_size=self.batch_size_test, num_workers=self.num_workers, pin_memory=True)
        else:
...
if __name__ == "__main__":
    ...
    data = ScanNet("/local-hdd/sjeske/Data/processed/ScanNet/", "data/configs/scannet/train_files_rel.json", "data/configs/scannet/test_files_rel.json", num_workers=4, batch_size_test=1)
    ...
    output_dir = "/local-hdd/sjeske/Data/processed/ScanNet/test_point_cloud/raw_points"
